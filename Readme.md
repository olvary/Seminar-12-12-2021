...
## Удаленные данные
### Из локального в удаленный репозитории.
Для того, чтобы переместить коммит из локального в удоленный репозитории необходимо использовать команду *git push origin master*. Используется она в папке с репозиторием следующим образом: *git push origin master*.

### Из удаленного в локальный репозитории.
Для того, чтобы переместить коммит из удаленного в локальный репозитории необходимо использовать команду *git pull origin master*.Используется она в папке с репозиторием следующим образом: *git pull origin master*.

### Синхронизация между удаленным и локальным репозиторием
Для синхронизации между удаленным и локальным репозиторием необходимо использовать команды:
*git remote add origin <адрес ссылки>*,
*git branch -M main*,
*git push -u origin main*.
Используется они в папке с репозиторием следующим образом:
*git remote add origin <адрес ссылки>*,
*git branch -M main*,
*git push -u origin main*.
###### The end
